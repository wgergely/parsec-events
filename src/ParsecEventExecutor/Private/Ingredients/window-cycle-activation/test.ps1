Context 'window.cycle-activation' {
    It 'captures the current foreground window and restores it after cycling' {
        $capture = Invoke-ParsecCoreIngredientOperation -Name 'window.cycle-activation' -Operation 'capture' -Arguments @{} -RunState @{}
        ...
        $apply = Invoke-ParsecCoreIngredientOperation -Name 'window.cycle-activation' -Operation 'apply' -Arguments @{ dwell_ms = 0; max_cycles = 4 } -RunState @{}
    ...
    It 'fails verification when the original foreground window is not restored' {
        ...
        $verify = Invoke-ParsecCoreIngredientOperation -Name 'window.cycle-activation' -Operation 'verify' -Arguments @{} -Prior $capture -RunState @{}
