...
function Import-ParsecIngredientModule {
    ...
    $existingFunctionNames = @(Get-ChildItem -Path Function: | Select-Object -ExpandProperty Name)
    . $libraryPath
    $loadedFunctionNames = @(Get-ChildItem -Path Function: | Select-Object -ExpandProperty Name)
    foreach ($functionName in @($loadedFunctionNames | Where-Object { $_ -notin $existingFunctionNames -and $_ -ne 'Get-ParsecIngredientOperations' })) {
        $functionInfo = Get-Item -LiteralPath ("Function:{0}" -f $functionName)
        Set-Item -LiteralPath ("Function:\script:{0}" -f $functionName) -Value $functionInfo.ScriptBlock
    }
    try {
